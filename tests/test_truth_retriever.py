...
def test_retriever_respects_max_chars(tmp_path: Path) -> None:
    db = TruthDatabase(tmp_path / "truth.db")
    db.insert_entries("lurenjia", 7, [truth_entry(7, "林默需要记住许青的提醒。", importance=0.9)])
    db.insert_entries("lurenjia", 6, [truth_entry(6, "这一条很长很长很长很长很长很长很长很长。", importance=0.9)])
    retriever = TruthRetriever(db)
...
def test_retrieve_excludes_current_chapter_truth(tmp_path: Path) -> None:
    db = TruthDatabase(tmp_path / "truth.db")
    db.insert_entries("lurenjia", 2, [truth_entry(2, "许青发现副楼门缝里有光。", importance=0.8)])
    db.insert_entries("lurenjia", 3, [truth_entry(3, "当前章尚未生成，不得进入提示词。", importance=1.0)])
    retriever = TruthRetriever(db)

    text = retriever.retrieve_for_prompt("lurenjia", 3, "许青副楼", max_entries=10, max_chars=1000)

    assert "[第2章]" in text
    assert "[第3章]" not in text
    assert "当前章尚未生成" not in text


def test_retrieve_excludes_future_chapters_from_all_sources(tmp_path: Path) -> None:
    db = TruthDatabase(tmp_path / "truth.db")
    db.insert_entries("lurenjia", 1, [truth_entry(1, "林默第一次听见翻译回声。", importance=0.9)])
    db.insert_entries("lurenjia", 2, [truth_entry(2, "许青要求林默暂时保密。", importance=0.8)])
    db.insert_entries("lurenjia", 3, [truth_entry(3, "当前章事实不得召回。", importance=1.0)])
    db.insert_entries("lurenjia", 4, [truth_entry(4, "未来章高重要事实不得召回。", category="world_rule", importance=1.0)])
    retriever = TruthRetriever(db)

    entries = retriever._select_entries("lurenjia", 3, "林默许青未来章高重要事实", max_entries=20)  # noqa: SLF001

    assert entries
    assert all(entry.chapter_no < 3 for entry in entries)
    assert {entry.chapter_no for entry in entries} == {1, 2}
